...
Выделил текст в любой программе, нажал `Ctrl+Alt+T` — рядом с курсором всплывает капсула с переводом. Второй хоткей заменяет выделенное переводом прямо в поле ввода. Движки по умолчанию бесплатные, без ключей и регистрации. Данные не покидают компьютер, кроме самого текста, который уходит в выбранный движок перевода.
...
- **Попап у курсора в любой программе** — браузер, мессенджер, редактор, терминал: захват выделения через эмуляцию `Ctrl+C`, как в QTranslate.
- **Замена текста на месте** — второй хоткей переводит и вставляет результат в то же поле, буфер обмена возвращается в исходное состояние, после вставки тост «Заменено: …».
...
**Перевести выделенное.** Выделите текст в любой программе → `Ctrl+Alt+T` → капсула у курсора разворачивается в карточку с переводом. `Esc`, клик вне окна или потеря фокуса закрывают попап.

**Заменить текст в поле.** Выделите текст → `Ctrl+Alt+R` → на месте выделения оказывается перевод, у курсора на 2 секунды всплывает капсула «Заменено: …». Буфер обмена после этого такой же, как до.
...
Приватность: текст уходит только выбранному движку перевода, больше никуда. Все данные — локально, в `%APPDATA%\com.utranslate.app\` (`settings.json` и `utranslate.db`), телеметрии и аналитики нет.
...
- **OCR области экрана** (`Ctrl+Alt+S`): оверлей выбора области → `Windows.Media.Ocr` → тот же попап.
