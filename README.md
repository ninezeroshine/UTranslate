...
**Заменить текст в поле.** Выделите текст → `Ctrl+Alt+R` → на месте выделения оказывается перевод, в трее на 2 секунды всплывает «Заменено: …». Буфер обмена после этого такой же, как до.
